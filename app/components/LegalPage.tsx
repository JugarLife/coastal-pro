    ...
    <>
      ...
      <main id="top" className="mx-auto max-w-[820px] px-6 lg:px-10 py-16 lg:py-24">
        ...
        <div className="border-l-2 border-brass pl-6 py-1 mb-14">
          <p className="label text-brass-ink mb-2">Draft — not yet reviewed</p>
          <p className="text-[15px] leading-[1.65] text-muted">
            ...
            Text in <span className="text-ink">[square brackets]</span> still needs your input.
          </p>
        </div>

        <h1 className="display display-light d2 text-ink mb-4">{title}</h1>
        ...
        <ol className="list-none">
          {clauses.map((c, i) => (
            <li key={c.heading} className="grid grid-cols-[auto_1fr] gap-x-7 pb-11 mb-11 border-b rule last:border-0">
              <span className="numeral text-[15px] text-ink/32 pt-1.5">
                {String(i + 1).padStart(2, '0')}
              </span>
              <div>
                <h2 className="display d4 text-ink mb-4">{c.heading}</h2>
        ...
        <div className="mt-16 pt-12 border-t rule flex flex-col sm:flex-row gap-3.5">
          <Link href="/" className="px-8 py-[14px] bg-ink text-paper text-[14px] font-medium text-center hover:bg-ink-deep transition-colors">
            Return to the site
          </Link>
          <Link href="/enquire" className="px-8 py-[14px] border border-ink text-ink text-[14px] font-medium text-center hover:bg-ink hover:text-paper transition-colors">
            Enquire about a membership
          </Link>
        </div>
      </main>

      <footer className="bg-ink-deep text-white/45">
